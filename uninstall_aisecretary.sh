...
# ============================================================
# AISecretary 卸载脚本
# 作用: 移除所有由 install_aisecretary.sh 安装的组件
# 注意: 默认会保留你的数据 (~/AISecretary 文件夹)
#      除非你明确选择删除
# ============================================================
...
# 2. 移除右键菜单
WORKFLOW="$HOME/Library/Services/存入 AI 秘书.workflow"
if [ -d "$WORKFLOW" ]; then
  echo -e "${BLUE}→ 移除右键菜单 '存入 AI 秘书'...${NC}"
  rm -rf "$WORKFLOW"
  echo -e "${GREEN}  ✓ 已移除${NC}"
