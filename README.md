...
直接html理解！👉[在线查看产品逻辑与可操作演示](https://luke20001024.github.io/Memento/Memento.html)
...
当前版本：**v0.8.7**。发行包包含安装器、Dashboard、Daily Review 协议及本地 Swift 源码；安装时会在当前 Mac 上编译需要的原生组件。
...
5. [`docs/Memento-cache-acceleration-design.md`](docs/Memento-cache-acceleration-design.md)：v0.8.7 快速缓存、4 路读取与多标签页约束。
...
- Chrome Dashboard 增加授权后的 last-known-good 快速缓存；暖启动先显示上次完整快照，再在后台核对。今天的文件完成后会立即替换缓存底稿并开放“今天”复制，周 / 月仍等待完整核对。缓存与目录句柄保存在扩展自己的本机 IndexedDB，最多复制 5 MiB 根目录日报正文，不包含照片、归档或 Daily Review；切换目录或明确撤权会令旧快照失效。
...
- 在 Web Locks 与 BroadcastChannel 可用时，同一时刻只允许一个 Memento 标签页执行核心扫描；其他页面只等待完整快照，不排队也不自动接管。扫描页未发布完整结果时，关闭其他 Memento 页面后手动刷新即可重新核对。部分结果不会覆盖完整缓存，旧目录任务也不能在切换后回写。
