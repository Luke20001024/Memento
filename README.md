...
当前版本：**v0.8.6**。发行包包含安装器、Dashboard、Daily Review 协议及本地 Swift 源码；安装时会在当前 Mac 上编译需要的原生组件。
...
- Chrome 目录恢复增加阶段提示和加载代次隔离；仅 IndexedDB 授权记录读写保留有界保护，保存失败不再阻断本次已授权目录，单个坏文件也不会拖垮整页。
- Chrome 文件扫描不再给不可取消的本地读取套硬超时或全局只读锁；偶发慢响应会继续等待并完整加载，真实的单文件异常才会局部降级。主记录仍先显示，Review/状态/Prompt 随后补充；归档写入与删除继续跨标签页串行。
- 归档写入使用跨标签页 Web Lock 串行化且同名自动改名；预览改为脚本零执行、外链零放行的 DOM 安全化路径。
